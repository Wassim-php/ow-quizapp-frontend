...
const QuestionDisplay = ({
  ...
  const handleAnswerSelect = (answerId) => {
    if (showFeedback && !isTimedMode) return;
  ...
  const handleNextClick = () => {
    ...
    setIsAnimating(true);
    setTimeout(() => {
      onAnswerSubmit(question.id, selectedAnswer, true);
    }, 500);
  };
  ...
  const getCardClass = (answerId) => {
    ...
    if (showFeedback && !isTimedMode) {
      if (checkIsCorrect(answerId)) {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-2xl p-6 md:p-10 transition-all duration-500 ease-out transform ${isAnimating ? 'opacity-0 translate-x-12' : 'opacity-100 translate-x-0'}`}>
      ...
      <div className="space-y-4">
        {question.answers.map((option, index) => (
          <div key={option.id} className={getCardClass(option.id)} onClick={() => handleAnswerSelect(option.id)}>
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 flex justify-between items-center">
        {(isTimedMode || showFeedback) && isAnswered ? (
          <button onClick={handleNextClick} className="px-6 py-3 text-lg font-medium rounded-lg bg-blue-600 text-white hover:bg-blue-700 flex items-center space-x-2 shadow-lg">
            <span>{currentQuestionIndex === questionData.length - 1 ? "Finish" : "Next Question"}</span>
            <ArrowRight className="w-5 h-5" />
          </button>
        ) : (
          <div className="text-gray-500 italic">{isTimedMode ? "Select an answer to continue" : "Select an answer to reveal feedback"}</div>
        )}
      </div>
